...
- `r length(unique(model_class$model))` Euro Hub models forecasting deaths and cases, classified by 3 coders
...
mym <- models |>
  tidyr::separate(target, 
                  into = c("location_target", "k"), sep = "_k", remove = FALSE) |>
  mutate(k = as.integer(k))  |>
  mutate(
    location = sub("^(..).*", "\\1", location_target),
    target = sub("^..(.*)", "\\1", location_target)
  ) |> 
  distinct(location, target, model, classification) |>
  mutate(location_target = paste0(location,"_", target)) |>
  mutate(location_target = fct_infreq(location_target)) |>
  group_by(location_target, classification) |>
  count() |>
  pivot_wider(names_from = location_target, values_from = n) |> 
  kable()
```
...
summary_scores_k_class <- ensemble_scores |>
  ...
  group_by(location, target, k, horizon, classification) |>
  summarise(
    n = n(),
...
summary_scores_k_class |>
  ...
  facet_grid(rows = vars(location, target),
             cols = vars(horizon),
             scales = "free_y") +
...
summary_scores_k_homog <- ensemble_scores|>
  ...
  group_by(location, target, k, horizon, homog) |>
  summarise(
    n = n(),
    meanrelskill = mean(scaled_rel_skill),
    q05relskill = quantile(scaled_rel_skill, 0.05),
...
ggplot(num_ensembles, aes(x = N, y = homog, fill = homog)) +
  ...
  facet_grid(location+target~ k, scales = "free") +
