...
ensdat <- fcdat |>
  ...
  select(model, location, target_type, quantile, forecast_date, horizon, true_value, prediction) |>
  distinct() |>
  score()


scores_fcdate <- ensdat |>
  summarise_scores(by = c("model", "location", "target_type", "forecast_date"))

scores_period <- ensdat |>
  DT(period_cats, on = c("forecast_date")) |>
  summarise_scores(by = c("model", "location", "target_type", "period_cat"))

# Save forecasts
fwrite(scores_fcdate, here("data", "scores_fcdate.csv"))

fwrite(scores_period, here("data", "scores_period.csv"))
