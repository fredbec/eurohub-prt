...
# Streamline to unique forecast metadata
# + count anomalies
#########!!!!!!!!!Own edits: added target_type grouping (since doing both cases and deaths)
######!! indicate rows where this was done with ##**##
metadata <- metadata |>
  DT(
    target_type %in% c("Cases", "Deaths")
  ) |>
  DT(, .(model, location, forecast_date, target_type, target_end_date, anomaly, target_type)) |>
  unique() |>
  DT(,
     `:=`(
     ...
     by = c("location", "forecast_date", "model", "target_type") ##**##
  ) |>
  DT(,
     .(model, location, forecast_date, target_type, target_end_date, anomaly, n_anomaly, n) ##**##
  ) |>
  unique()
