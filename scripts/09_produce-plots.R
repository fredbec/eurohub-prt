...
tileplot <- function(horizons = c(1,2)){
  ...
  scores_individual <- data.table::fread(here("output", "selection-ensemble", "forecasts", paste0("scores-selection-ens", suffix, ".csv")))
  scores_average <- data.table::fread(here("output", "selection-ensemble", "forecasts", paste0("avg-scores-selection-ens", suffix, ".csv")))
...
pdf(here("plot_results", "bestperform_tileplot_allhor.pdf"), width = 13, height = 4.25)
...
pdf(here("plot_results", paste0("ens_comb_", "median_ensemble",".pdf")), width = 12, height = 8)
median_plot <- make_recomb_plot("median_ensemble")
print(median_plot)
dev.off()

pdf(here("plot_results", paste0("ens_comb_", "mean_ensemble",".pdf")), width = 12, height = 8)
mean_plot <- make_recomb_plot("mean_ensemble")
print(mean_plot)
dev.off()


pdf(here("plot_results", paste0("ens_comb_", "median_ensemble_annotated",".pdf")), width = 12, height = 8)
